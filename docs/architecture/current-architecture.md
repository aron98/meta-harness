...
`packages/core/src/candidates/mutation-catalog.ts` enumerates the current deterministic mutation records. `packages/core/src/candidates/search-config.ts` can also parse custom mutation arrays, bounded combination size, candidate caps, and default-mutation inclusion. `packages/core/src/candidates/objective-config.ts` owns configurable scoring weights and penalties for packet completeness, route hit rate, checklist coverage, expected tag hit rate, selected records, and selected commands. `packages/core/src/candidates/candidate-policy.ts` projects candidates into the existing packet-preparation policy input so retrieval scoring, task classification, context limits, and verification checklist construction can be tuned without changing default behavior. `packages/core/src/candidates/candidate-paths.ts` keeps run artifacts under safe path segments such as `data/candidate-runs/run-001/candidates/baseline/candidate.json`.
...
`packages/core/src/candidates/evaluate-candidate.ts` evaluates one candidate by reusing `evaluatePacketBenchmarks()` with the candidate policy threaded into packet preparation. Candidate-level context limits override run-level `maxMemories` and `maxArtifacts` when present, including explicit zero limits. `packages/core/src/candidates/search-objective.ts` owns the scalar score and returns score contributions for each objective term. `packages/core/src/candidates/run-candidate-search.ts` filters supplied fixtures to `split === "train"`, evaluates explicit candidates or generated mutation combinations, persists run artifacts, and selects exactly one winner with deterministic tie-breaking. `packages/core/src/candidates/validate-held-out.ts` evaluates only that selected winner on `split === "held-out"` fixtures and records held-out metrics in `selection.json` without changing the winner. `packages/core/src/candidates/export-candidate-policy.ts` reads the stored selection plus winner candidate snapshot and writes only the requested runtime policy artifact.

The CLI resolves `fixturesFile`, `candidatesFile`, `searchConfigFile`, `mutationsFile`, and `objectiveFile` relative to the process working directory. `searchConfigFile` carries the full generated-combination config, while `mutationsFile` remains a shorthand for standalone mutation arrays. This keeps examples and scripts predictable: run from the repo root or pass absolute paths. The export step is deliberately safe. It does not patch OpenCode config, global user config, or any runtime adapter state.
